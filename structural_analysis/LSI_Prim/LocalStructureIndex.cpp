...
std::vector<double> LocalStructureIndex::LSI() {
   ...
   for (unsigned int i = 0; i < iN; i++) {
      ...
      int nLSI = 0;
      for (unsigned int neigh = 0; neigh < m_vvNeighList[i].size(); neigh++) {
	 if ( m_vvRij[i][neigh] <= m_dLSI_cut ) {
	    nLSI += 1;
	 }
      }
      if ((int)(m_vvNeighList[i].size()) == nLSI) {
	 nLSI--;
      }
...
void LocalStructureIndex::LSI(const double dCut) {
   ...
   printf("%d\n", iN);
   printf("Atom. Timestep: 1\n");
   for (unsigned int i = 0; i < iN; i++) {
      if (vLSI[i] < dCut) {
	 printf("O %f %f %f\n", oWaters[i].pdO[0], oWaters[i].pdO[1], oWaters[i].pdO[2]);
      }
      else {
	 printf("N %f %f %f\n", oWaters[i].pdO[0], oWaters[i].pdO[1], oWaters[i].pdO[2]);
      }
...
int LocalStructureIndex::CreateNeighborList() {
   ...
   for (unsigned int i = 0; i < iN; i++) {
      ...
      double dRmin = 0; 
      for (unsigned int num = 0; num < vNeigh.size(); num++) {
	 double dRnext = m_dRooCut_2;
	 for (unsigned int neigh = 0; neigh < vNeigh.size(); neigh++) {
   ...
   for (unsigned int i = 0; i < vvRij_2.size(); i++) {
      for (unsigned int j = 0; j < vvRij_2[i].size(); j++) {
