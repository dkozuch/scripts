...
print "Number of round trips: " + str(len(round_trip_times))
...
for i in range(0,np.shape(replica_assignment)[1]):
	count = 0
	for j in range(1,np.shape(replica_assignment)[0]):
		if replica_assignment[j,i] != 0 and replica_assignment[j-1,i] != 0 and replica_assignment[j,i] != replica_assignment[j-1,i]:
			count = count + 1
	round_trips_per_replica.append(count)
